        ...
        troubleShooting: [
            {
                ...
                issue: '스포티파이 Web API와 통신하면 서버에서 웹 액세스 토큰과 만료 시간이 내려옵니다. 하지만 이때 내려오는 값은 단순히 유효 시간일 뿐, 실제로 토큰이 언제 만료되는지에 대한 시점은 제공되지 않습니다. 이로 인해 만료된 토큰을 API 통신에 사용하게 되면 401 Unauthorized 에러가 발생할 수 있습니다. UX 측면에서 불편함을 유발합니다',
                solution: [
                    {
                        content:
